...
def get_patient_trace(trace_df, step, current_episode, current_episode_step, current_action, current_state, current_obs, 
                      ...
                      current_done, current_patient_info,final_s_outcome,final_r_outcome):
    ...
    current_step_info_dict = {'step':step, 'episode':current_episode, 'episode_step':current_episode_step, 
...
parser.add_argument('--arg_patient_type', '-arg_patient_type', help='patient type', default='adult', type=str, required=True)
...
parser.add_argument('--arg_train_step', '-arg_train_step', help='train step', default=1000000, type=int,required=True)
parser.add_argument('--arg_callback_step', '-arg_callback_step', help='callback step', default=50000, type=int,required=True)
parser.add_argument('--arg_lr', '-arg_lr', help='learn rate', default=0.001, type=float)
parser.add_argument('--arg_test_step', '-arg_test_step', help='test step', default=3400, type=int)
...
print(device)
# train model with different reward functions
# adult 5, child 5, adolescent 4
patient_id = args_ppo_diabetic.arg_patient_id
...
register(
    id='simglucose-{patient_type}{patient_id}-v0'.format(patient_type=patient_type, patient_id=patient_id),
    entry_point='simglucose.envs:T1DSimEnv',
    kwargs={'patient_name': '{patient_type}#{id}'.format(patient_type=patient_type,id=str(patient_id).zfill(3)),
            'reward_fun': reward_func_dict[reward_fun_id]}
)

ENV_NAME = 'simglucose-{patient_type}{patient_id}-v0'.format(patient_type=patient_type, patient_id=patient_id)
...
# Hyperparameters
render = False
trace_file_folder = '/Counterfactual_Explanation/diabetic_example/trained_ppo/patient_{patient_type}_{patient_id}_t_{arg_train_step}_c_{arg_callback_step}_lr_{arg_lr}'.format(patient_type=patient_type,
                                                                                            patient_id=patient_id, arg_train_step=args_ppo_diabetic.arg_train_step,
                                                                                            arg_callback_step=args_ppo_diabetic.arg_callback_step,arg_lr=args_ppo_diabetic.arg_lr)
...
train_step=args_ppo_diabetic.arg_train_step

# Separate evaluation env, wrapped the same way as the training env
...
#save_vec_normalize = SaveVecNormalizeCallback(save_freq=1, save_path=best_result_folder)
...
checkpoint_callback = CheckpointCallback(
  ...
  name_prefix="ppo_diabetic_model_{noise_type}".format(noise_type=noise_type),
...
# set training log
tmp_path = "{save_folder}/logs/{noise_type}/{train_step}_{lr}/train_log".format(save_folder=save_folder,
                                                                                          noise_type=noise_type,
                                                                               train_step=train_step,lr=lr)
# set up logger
new_logger = configure(tmp_path, ["stdout", "csv", "tensorboard"])
# train model and save checkpoints and logs
#model = DDPG("MlpPolicy", env, action_noise=action_noise, verbose=10, learning_rate=lr)
model = PPO("MlpPolicy", env, verbose=10, learning_rate=lr)
...
print('Begin testing.')
test_step = args_ppo_diabetic.arg_test_step
result_col = ['id', 'hypoPercent', 'hyperPercent', 'TIR', 'step', 'reward_func']
results_path = '{folder}/medical_metrics_results_{patient_type}_patient_{patient_id}_rf_{rf}.csv'.format(
                                                      folder=trace_file_folder,patient_type=patient_type,
                                                      patient_id=patient_id,
                                                      rf=reward_fun_id)
result_df = pd.DataFrame(columns=result_col)

# Load the saved statistics, env and model
model_path='{best_result_folder}/best_model'.format(best_result_folder=best_result_folder)
stats_path='{best_result_folder}/vecnormalize.pkl'.format(best_result_folder=best_result_folder)

test_env = Monitor(gym.make(ENV_NAME)) 
#test_env = NormalizeActionWrapper(test_env)
...
trace_file_path = '{folder}/patient_trace_{patient_type}#{patient_id}_rf_{rf}_step_{step}.csv'.format(folder=trace_file_folder,
                                                                                 patient_type=patient_type,patient_id=patient_id,
                                                                                rf = reward_fun_id,step=test_step)
# Iterate over the number of epochs for evaluation
epochs = 5
# store the past trace of this epoch
trace_df = pd.DataFrame(columns=['step', 'episode', 'episode_step', 'action', 'state',
...
for epoch in range(epochs):
    ...
    for t in range(test_step):
        ...
        #print(action, observation,observation_new, reward, episode_return, done, info)
        ...
        if (t == test_step - 1) or terminal:
            ...
            trace_df.final_s_cgm_outcome[trace_df.episode == num_episodes] = [final_s_outcome_this_eps[0]]*episode_length
            trace_df.final_s_cho_outcome[trace_df.episode == num_episodes] = [final_s_outcome_this_eps[1]]*episode_length
            trace_df.final_s_roc_outcome[trace_df.episode == num_episodes] = [final_s_outcome_this_eps[2]]*episode_length
            trace_df.final_s_insulin_outcome[trace_df.episode == num_episodes] = [final_s_outcome_this_eps[3]]*episode_length
            trace_df.final_r_outcome[trace_df.episode == num_episodes] = [final_r_outcome_this_eps]*episode_length
    ...
    patient_type_list = [patient_type]
    patient_id_num_list = [patient_id]
